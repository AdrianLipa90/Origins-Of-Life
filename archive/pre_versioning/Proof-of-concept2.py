...
origin_merged.py
Merged tool:
 - sweep_v3 reproduction (day/night + tides + EM_pattern)
 - Universal 5-scenario origin-of-life simulator with KÄHLER-like topology (Berry–Euler analog)

Usage:
    python origin_merged.py --mode sweep_v3    # run sweep v3 reproduction
    python origin_merged.py --mode scenarios   # run 5 scenarios with topo
    python origin_merged.py --mode both        # run both (careful: heavy)
    python origin_merged.py -h                 # show help
...
def run_once_sweep_v3(
    Nx=96, Ny=96, hours=30.0, dt_h=0.05, seed=42,
...
def run_sweep_v3_and_save(outdir='sweep_v3_outputs'):
    ensure_dir(outdir)
        ...
        for mu in mu_list:
            for dayf in dayf_list:
                ...
                metrics = run_once_sweep_v3(
                    ...
                    hours=30.0, dt_h=0.05, seed=4748+combo_id, Nx=96, Ny=96
                )
...
# (adapted from the longer implementation)
# ---------------------------
...
# scenarios definitions (values tuned as example)
...
class UniversalOriginSimulator:
    ...
    def step_degradation(self):
        ...
        probs = k_deg * temp_factor * self.dt_h * local_mod
...
# ---------------------------
def main():
    parser = argparse.ArgumentParser(description="Merged origin-of-life tools: sweep_v3 + 5-scenario topo simulator")
    parser.add_argument('--mode', choices=['sweep_v3', 'scenarios', 'both'], default='both', help='Which module to run')
    parser.add_argument('--outdir', type=str, default='outputs', help='Base output directory')
    parser.add_argument('--nx', type=int, default=96, help='Grid Nx')
    parser.add_argument('--ny', type=int, default=96, help='Grid Ny')
    ...
    if args.mode in ('sweep_v3', 'both'):
        ...
        run_sweep_v3_and_save(outdir=sweep_outdir)
