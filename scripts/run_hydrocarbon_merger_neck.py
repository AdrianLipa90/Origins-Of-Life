...
from origins.exobiology.hydrocarbon_merger_neck import (
    diagnostic_manifest,
    run_matched_hydrocarbon_merger_neck,
)
...
def main() -> int:
    ...
    parser.add_argument("--outdir", default="hydrocarbon_merger_neck_artifact")
    ...
    rows = [
        row.as_record()
        for row in run_matched_hydrocarbon_merger_neck(
            SCENARIO_D,
            seeds=args.seeds,
    ...
    json_path = outdir / "hydrocarbon_merger_neck_v01.json"
    csv_path = outdir / "hydrocarbon_merger_neck_v01.csv"
