...
from origins.exobiology.hydrocarbon_bridge_flux import (
    diagnostic_manifest,
    run_matched_hydrocarbon_bridge_flux,
)
...
def main() -> int:
    ...
    parser.add_argument("--outdir", default="hydrocarbon_bridge_flux_artifact")
    ...
    rows = [
        row.as_record()
        for row in run_matched_hydrocarbon_bridge_flux(
            SCENARIO_D,
            seeds=args.seeds,
    ...
    json_path = outdir / "hydrocarbon_bridge_flux_v01.json"
    csv_path = outdir / "hydrocarbon_bridge_flux_v01.csv"
